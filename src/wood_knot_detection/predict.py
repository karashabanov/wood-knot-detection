...
def load_test_manifest(path: Path) -> List[Path]:
    with open(path, "r") as file:
        return [Path(line.strip()) for line in file.readlines() if line.strip()]


if __name__ == "__main__":
    # Add arguments
    parser = argparse.ArgumentParser()
    parser.add_argument("--seed", type=int, required=True, help="Dataset split seed.")
    parser.add_argument(
        "--run",
        type=str,
        required=True,
    ...
    dataset = WoodKnotDataset(image_dir=image_dir, label_dir=label_dir)

    # LookUp for Ground Truth samples (raw dataset)
    samples_by_image = {sample.image_path: sample for sample in dataset.samples}
    ...
    if args.test_dir is not None:
        ...
        output_dir = (
            Path(config.get("output").get("directory"))
            / f"seed_{args.seed}"
            / f"{args.run}"
        )
    ...
    for board_index, board_samples in boards.items():
        ...
        board.save_board_image(
            output_dir=output_dir, board_index=board_index, image=stitched_board
        )
