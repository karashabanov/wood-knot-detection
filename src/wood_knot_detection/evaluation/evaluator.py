...
class Evaluator:
    @staticmethod
    def evaluate(
        prediction_samples: List[PredictionSample], iou_threshold: float = 0.5
    ) -> EvaluationResult:
        true_positives = 0
        false_positives = 0
        false_negatives = 0
        ...
        for sample in prediction_samples:
            ...
            _tp, _fp, _fn, detection_matches = Evaluator._match_detections(
                sample.annotations, sample.detections, iou_threshold
            )
        ...
        mean_iou = (
            total_true_positive_iou / true_positive_count
    ...
    def save(
        ...
        with open(output_path, "w", encoding="utf-8") as file:
            json.dump(asdict(evaluation_result), file, indent=4)
